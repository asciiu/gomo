...
import (
	...
	talib "github.com/asciiu/go-talib"
...
func TestGetCandle(t *testing.T) {
	...
	request := protoBinance.CandleRequest{
		MarketName: "ADA-BTC",
	}
	...
	fmt.Println(len(response.Data.Candles))
	close := make([]float64, 0)
	total := 0.0
	for i, c := range response.Data.Candles {
		if i >= 20 {
			break
		}
		fmt.Println(c.CloseTime)
		total += c.Close
		close = append(close, c.Close)
	}

	//fmt.Println(talib.Ema(close, 1))
	fmt.Println(total / 20)
	fmt.Println(talib.Sma(close, 20))

	assert.Equal(t, "success", response.Status, response.Message)
	assert.Equal(t, 1000, len(response.Data.Candles), "should be 500 candle data")
	//fmt.Println(len(testClose))
...
func TestEventConversion(t *testing.T) {
	...
	assert.Equal(t, c.InitialCurrencyBalance, event.CommittedCurrencyAmount, "initial balance no match")
